...
what the scene demonstrably contains, and localise defects to specific stages.
...
\Cref{tab:results} summarises scene composition. The mesh/card/billboard split is
...
occlusion boundaries; collapsing them removed the concentric artefacts on the
...
Colour-vetoed grass masking correctly excludes snowfields that type identically to
...
were able to localise the failure to an \emph{assumption} rather than a model.
...
box and an open-vocabulary detector will localise almost any label inside it.

The consequence for reconstruction is direct: SAM~3D given a flat rock fragment
returns a flat sheet (normalised extents $[1.00, 0.049, 0.425]$), the shape gates
...
more than a metre above the median --- the ocean draped up the cliffs.
...
localise cleanly. Preserving measured data as hard constraints means the pipeline
